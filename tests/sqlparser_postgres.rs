...
fn parse_like() {
    pg().one_statement_parses_to(
        "SELECT * FROM foo WHERE bar LIKE '[a-c]%[f-g]' ESCAPE '\\'",
        "SELECT * FROM foo WHERE bar LIKE '[a-c]%[f-g]' ESCAPE '\\'",
    );

    match pg().verified_stmt("SELECT * FROM foo WHERE bar LIKE '[a-c]%[f-g]' ESCAPE '\\'") {
...
#[test]
fn parse_not_similar_to() {
    pg().one_statement_parses_to(
        "SELECT * FROM foo WHERE bar NOT SIMILAR TO '[a-c]%[f-g]' ESCAPE '\\'",
        "SELECT * FROM foo WHERE bar NOT SIMILAR TO '[a-c]%[f-g]' ESCAPE '\\'",
    );

    match pg().verified_stmt("SELECT * FROM foo WHERE bar NOT SIMILAR TO '[a-c]%[f-g]' ESCAPE '\\'")
    {
        Statement::Query(ref query) => {
            match &*query.body {
                SetExpr::Select(select) => {
                    match &select.selection {
                        Some(Expr::NotSimilarTo {
                            escape_char: Some('\\'),
                            ..
                        }) => {
                            // good
